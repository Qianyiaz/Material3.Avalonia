...
internal sealed class RoundedCornerHelper
{
    ...
    public RoundedCornerHelper(Vector2 p0, Vector2 p1, Vector2 p2, CornerRounding? rounding)
    {
        ...
        if (d01 > 0f && d21 > 0f)
        {
            ...
            _sinAngle = Sqrt(Math.Clamp(1f - _cosAngle * _cosAngle, 0f, 1f));
        }
        else
        {
            _d1 = Vector2.Zero;
    ...
    public float ExpectedRoundCut =>
        _sinAngle > 1e-3f ? _cornerRadius * (_cosAngle + 1f) / _sinAngle : 0f;
        ...
        Vector2 circleCenter, float r)
    {
        ...
        var mid = Vector2.Lerp(circleSegIntersection, (circleSegIntersection + otherCircleSegIntersection) * 0.5f, smooth);
