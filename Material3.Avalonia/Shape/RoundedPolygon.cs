...
public sealed class RoundedPolygon
{
    ...
    public RoundedPolygon Transformed(Func<Vector2, Vector2> transform)
    {
        ...
        foreach (var f in Features)
        {
            ...
            cc.AddRange(f.Cubics.Select(cubic => new Cubic(transform(cubic.A0), transform(cubic.C0), transform(cubic.C1), transform(cubic.A1))));
    ...
    public static RoundedPolygon RegularPolygon(int n, float radius = 1f, float startAngle = 0f, float cx = 0f, float cy = 0f,
        ...
        float cx = 0f, float cy = 0f)
    {
        ...
        for (var i = 0; i < verticesPerRadius; ++i)
        {
            ...
            arr[idx++] = v1.X;
            arr[idx++] = v1.Y;
            var v2 = RadialToCartesian(innerRadius, 2 * PI * (2 * i + 1) / (2  * verticesPerRadius), new Vector2(cx, cy));
            arr[idx++] = v2.X;
            arr[idx++] = v2.Y;
        }
